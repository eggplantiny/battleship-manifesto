...
export class WMALLMSalvageStrategy implements Strategy {
  ...
  private readonly decisionOllama: OllamaClient;
  private readonly explanationOllama?: OllamaClient;
  ...
  constructor(
    ...
    private readonly explainModel: string | undefined,
    candidateQuestions: number = 10,
    targetQuestions: number = 12,
    private readonly llmCandidates: number = 3,
  ) {
    this.decisionOllama = new OllamaClient(decisionModel);
    this.explanationOllama = explainModel ? new OllamaClient(explainModel) : undefined;
    ...
    this.decisionStage = decorateStage(createSalvageDecisionLLMStage(this.decisionOllama, this.decisionModel));
  ...
  private async maybeExplain(
    ...
    if (!this.explanationOllama || !this.explainModel || !this.explanationSystemPrompt) {
      return {};
    }
    return explainDecisionChoice(
      this.explanationOllama,
      this.explainModel,
