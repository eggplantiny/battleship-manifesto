...
} from "../../runtime/simulation-world.js";
import {
...
} from "../../runtime/world-belief-summary.js";
...
abstract class ReflectiveStrategyBase implements Strategy {
  ...
  protected async ensureReflectionConfigured(ctx: TurnContext): Promise<void> {
    ...
    const currentMinRevisionDelta = asNumber(
      ctx.bridge.data.minRevisionDelta,
      DEFAULT_MIN_REVISION_DELTA,
    );
    const currentAllowLooseCoarseRevision = readBoolean(
      ctx.bridge.data.allowLooseCoarseRevision,
      true,
    );
    const nextMinRevisionDelta = this.minRevisionDelta ?? currentMinRevisionDelta;

    if (
      currentMinRevisionDelta !== nextMinRevisionDelta ||
      currentAllowLooseCoarseRevision !== this.allowLooseCoarseRevision
    ) {
      await ctx.bridge.dispatch(
        "configureCounterfactualRevision",
        nextMinRevisionDelta,
  ...
  protected logRevision(ctx: TurnContext, applied: AppliedRevisionMeta): void {
    ctx.logger?.log({
      ...
      data: {
        ...
        lastRevisionDelta: asNumber(ctx.bridge.data.lastRevisionDelta, 0),
...
export class CRAStrategy extends ReflectiveStrategyBase {
  readonly name = "cra";
  readonly policyName = "reflective-counterfactual-v1";
...
function buildRevisionPreviewValues(
  ...
  return {
    ...
    reopenLocalProbePreviewValue: evaluatePolicyPreview(
      ctx,
      summary,
      bestShoot,
      {
    ...
    confidenceCollapseReprobePreviewValue: evaluatePolicyPreview(
      ctx,
      summary,
      bestShoot,
      {
...
function evaluatePolicyPreview(
  ...
  const currentPolicy = readCurrentPolicyState(ctx);
  ...
  const bestQuestionValue = questionMode !== "none" && questionBudgetOpen
    ? evaluateBestQuestionValueForMode(ctx, summary, questionMode, candidateQuestions)
    : 0;
  ...
  const immediateActionValue = preferQuestion ? bestQuestionValue : bestShoot.boardValue;
  const actionSpaceReopenBonus = computeActionSpaceReopenBonus(
    ctx,
    summary,
    bestShoot,
    currentPolicy,
...
function evaluateBestQuestionValueForMode(
  ...
  if (mode === "none" || asNumber(ctx.bridge.data.questionsRemaining, 0) <= 0) {
    return 0;
  }

  let bestQuestionValue = 0;
  const questions = selectQuestionsForMode(mode, ctx.askedQuestions).slice(0, candidateQuestions);
  for (const question of questions) {
    try {
      const detail = evaluateWorldQuestionDetailed(
        ctx.bridge,
  ...
  return bestQuestionValue;
}

function computeActionSpaceReopenBonus(
  ctx: TurnContext,
  summary: WorldBeliefSummary,
  bestShoot: { hitProb: number; boardValue: number },
  ...
  let reopenedQuestionValue = 0;

  const currentLocalOpen = isQuestionBudgetOpenForPolicy(ctx, "local", currentPolicy);
  const nextLocalOpen = isQuestionBudgetOpenForPolicy(ctx, "local", nextPolicy);
  if (!currentLocalOpen && nextLocalOpen) {
    reopenedQuestionValue = Math.max(
      reopenedQuestionValue,
      evaluateBestQuestionValueForMode(ctx, summary, "local", candidateQuestions),
    );
  }

  const currentLateOpen = isQuestionBudgetOpenForPolicy(ctx, "late", currentPolicy);
  const nextLateOpen = isQuestionBudgetOpenForPolicy(ctx, "late", nextPolicy);
  if (!currentLateOpen && nextLateOpen) {
    reopenedQuestionValue = Math.max(
      reopenedQuestionValue,
      evaluateBestQuestionValueForMode(ctx, summary, "late", candidateQuestions),
    );
  }
